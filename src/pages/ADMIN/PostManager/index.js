  ...
  const columns = [
    {
      ...
      render: (val, record, idx) => (
        <Tooltip title={val} mouseLeaveDelay={0}>
          <div className="max-line3">{val}</div>
        </Tooltip>
      ),
    },
    {
      title: "Danh mục",
      dataIndex: "ListCategoryPostNamend",
      key: "ListCategoryPostNamend",
      width: 180,
      render: (value, record) => (
        <Tooltip
          title={
            <>
              <div className="fw-600">{record?.CategoryPostName}</div>
              <div>
                {value.map(i => (
                  <div>{`- ${i}`}</div>
                ))}
          ...
          mouseEnterDelay={0.3}
        >
          <div className="max-line1 fw-600">{record?.CategoryPostName}</div>
          <div>
            {value.map(i => (
              <div className="max-line1">{`- ${i}`}</div>
            ))}
          </div>
        </Tooltip>
      ),
