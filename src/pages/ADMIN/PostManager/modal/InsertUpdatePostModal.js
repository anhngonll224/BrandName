...
import { convertTreeDataUser, getListComboByKey, normFile } from "src/lib/utils"
...
const InsertUpdatePostModal = ({
  ...
  const getListAuthor = async () => {
    try {
      ...
      const treeData = convertTreeDataUser(res?.Object)
      setListAuthor(treeData)
    } finally {
      setLoading(false)
    }
  }
          ...
          <SpinCustom spinning={loading}>
            <Row gutter={[16, 16]}>
              <Col span={14}>
                ...
                <Row gutter={16}>
                  <Col span={btnShow?.IsUpdateRoyalties ? 8 : 12}>
                    <Form.Item label="Tác giả" name="AuthorID">
                      <TreeSelect
                        ...
                        placeholder="Chọn đối tượng hưởng"
                        allowClear
                      >
                        {listAuthor.length > 0 &&
                          listAuthor.map(item => (
                  ...
                  <Col span={btnShow?.IsUpdateRoyalties ? 8 : 12}>
                    <Form.Item label="Tác giả cũ" name="Author">
                      <Input placeholder="tác giả cũ" />
                    </Form.Item>
                  </Col>
                  {btnShow?.IsUpdateRoyalties && (
                    <Col span={8}>
