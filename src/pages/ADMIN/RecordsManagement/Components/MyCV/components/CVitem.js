...
import ModalListSign from "src/pages/USER/MyCV/modal/ModalListSign"
...
const CVitem = ({
  ...
  useEffect(() => {
    if (!!open?.DossierID) {
      stateOne(open?.DossierID)
    }
  }, [open?.DossierID])

  const stateOne = async DossierID => {
    try {
      // const values = await form.validateFields(["RegisterIdentify"])
      // const res = await UserService.getUserByIdentification(
      //   values?.RegisterIdentify,
      // )
      const ValueDetail = await Dossier.getDetail(
        DossierID,
        // values?.RegisterIdentify,
      )
      const Value = ValueDetail?.Object?.Data
      if (ValueDetail?.isError) return
      form.setFieldsValue({
        ...Value,
        RepresentativeBirthDay: dayjs(Value?.RepresentativeBirthDay),
        ManagerBirthDay: dayjs(Value?.ManagerBirthDay),
        // IdentifyName:default.IdentifyName,
        IdentifyName: Value?.IdentifyName,
        //Note@
        //linh vực hoạt động
        ...
        //Cá nhân đăng ký
        RegisterIdentify: Value?.RepresentativeIdentify,
        // Tên cá nhân
        RegisterName: Value?.RepresentativeName,
        // Email
        RegisterEmail: Value?.RegisterEmail,
        // Điện thoai
        RegisterPhone: Value?.RepresentativePhone,
        // Fax
        RegisterFax: Value?.RepresentativeFax,
        // địa chỉ
        RegisterAddress: Value?.RepresentativeAddress,
        // mục đích sử dụng
        PurposeUse: Value?.PurposeUse,
        // Hình thức sử dụng
        FormUse: Value?.FormUse,
        // Sản phẩm
        ProductAttached: Value?.ProductAttached,
      })
      setFieldCCCD({})
      // setTwoText(res?.Object?.Data?.FullName)
    } finally {
    ...
    <MyCVStyle>
      ...
      >
        ...
        <Row gutter={[16, 16]}>
            ...
            <Row gutter={[16, 0]}>
                    ...
                    <Col flex={"auto"} style={{ width: 0 }}>
                      ...
                      >
                        <Input
                          ...
                          disabled={
                            !!(!!open?.DossierID && open?.Procedure !== 1) ||
                            !!open?.IsChangeInfor ||
                            !!open?.IsAdditional ||
                            !!open?.IsReIssue
                          }
              ...
              <Col span={12}>
                <Form.Item
                  ...
                  name="FormUse"
                >
                  <Select placeholder="Chọn" disabled={!!open?.IsAdditional}>
                    {getListComboByKey(
          ...
          <Col span={24}>
            <div className="box-infor " style={{ height: "100%" }}>
              <Row gutter={[16, 0]}>
                  ...
                  <Row gutter={16}>
                    <Col flex="auto" style={{ width: 0 }}>
                      ...
                      >
                        <Input
                          disabled={
                            !!open?.IsChangeInfor || !!open?.IsAdditional
                          }
                          placeholder="Nhập"
                          allowClear
                        />
                ...
                <Col span={24}>
                  <Form.Item
                    label={`Tên ${!!(type === 2) ? "cá nhân" : "tổ chức/doanh nghiệp"
                      }`}
                    name="RegisterName"
                    required
                    rules={[
                  ...
                  >
                    <Input
                      placeholder="Chọn"
                      disabled={!!open?.IsChangeInfor || !!open?.IsAdditional}
                      onChange={value => {
                ...
                <Col span={12}>
                  ...
                  >
                    <Input placeholder="Chọn" disabled={!!open?.IsAdditional} />
                  </Form.Item>
                </Col>
                <Col span={6}>
                  <Form.Item
                  ...
                  >
                    <Input placeholder="Nhập" disabled={!!open?.IsAdditional} />
                  </Form.Item>
                </Col>
          ...
          <Col span={24}>
            <InforUser
              IsAdditional={open?.IsAdditional}
